...
OTHER_LAYERS = [nn.Linear, nn.Conv1d, nn.Conv2d, nn.Conv3d, nn.ConvTranspose1d, nn.ConvTranspose2d, nn.ConvTranspose3d,
                ops.DeformConv2d, nn.Parameter, nn.MaxPool2d, nn.AdaptiveAvgPool2d, nn.Dropout]
NON_LAYERS = []
...
def scheduler_from_config(scheduler_config, optimizer, epoch_length):
    assert scheduler_config["type"] in ("linear", "step", "poly", "multistep", "multistep_multigamma")
    ...
    elif scheduler_config['type'] == "multistep_multigamma":
        scheduler = lr_scheduler.MultiStepMultiGammaLR(optimizer, params["milestones"], params["gamma"])
    else:
...
def replace_bn(model, norm):
    def apply_replace_bn(m):
        for var_name in dir(m):
            ...
            if type(target_attr) == torch.nn.BatchNorm2d:
                ...
                setattr(m, var_name, norm(num_features, eps, momentum, affine))
        ...
        for var_name, children in m.named_children():
            apply_replace_bn(children)

    model.apply(apply_replace_bn)
    return model
